...
export class GeminiService {
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        this._fetchApiKeyFromSecrets().then(key => { // Renamed for clarity and to use the new fetch-only method
            if (key) {
                this.currentApiKey = key; // Set the instance variable here
    ...
    /**
     * Fetches the API key from secrets or environment variables.
     * This method does NOT set the instance's `currentApiKey`.
     */
    private async _fetchApiKeyFromSecrets(): Promise<string | undefined> {
        let apiKey = await this.context.secrets.get(API_KEY_SECRET_ID);
        if (apiKey) {
            console.log("GeminiService: API Key fetched from VS Code secrets.");
            return apiKey;
        }
        // Fallback to environment variable if not in secrets
        apiKey = process.env.GEMINI_API_KEY;
        if (apiKey) {
            console.log("GeminiService: API Key fetched from environment variable.");
            // Optionally store it in secrets for future use: await this.context.secrets.store(API_KEY_SECRET_ID, apiKey);
        }
        return apiKey;
    }
    ...
    public async getApiKey(): Promise<string | undefined> {
        // Only attempt to load from secrets if currentApiKey is not already set (e.g., by constructor or setApiKey)
        if (this.currentApiKey === undefined) {
            const loadedKey = await this._fetchApiKeyFromSecrets();
            // Set currentApiKey only if a key was successfully fetched and it wasn't already set
            if (loadedKey !== undefined) this.currentApiKey = loadedKey;
        }
